...
def alter_graph(graph, bad_user_set):
    """
    ...
    start = time.time()
    correct_label, label_score = calc_target_output(graph)
    end = time.time()
    ...
    for bad_user in tqdm(bad_user_set):
        for tweet in tqdm(tweet_set):
            if int(bad_user) not in graph[str(tweet)] and int(tweet) not in graph[str(bad_user)]:
                ...
                correct_label2, label_score2 = calc_target_output(new_graph)
                count += 1
                print('finish {} calculations'.format(count))
                label_attack = correct_label - correct_label2
                value_attack = label_score - label_score2

                if best_label_attack < label_attack:
                    best_label_attack = label_attack
                    label_chosen_edge = (bad_user, tweet)
                if best_value_attack < value_attack:
                    best_value_attack = value_attack
                    value_chosen_edge = (bad_user, tweet)
    
    if best_label_attack > 0:
        chosen_edge = label_chosen_edge
        return chosen_edge, best_label_attack, None 
    else:
        chosen_edge = value_chosen_edge
        return chosen_edge, None, best_value_attack


@torch.no_grad()
def calc_target_output(graph_dict):
    model.eval()
    print('jump into testing calc_target_output')
    start = time.time()
    train_loader, test_loader = get_dataloader(args.batch_size, train_indices, test_indices, graph_dict)
    ...
    for data in test_loader:
        ...
        if not len(all_pred):
            ...
            all_pred = np.vstack((all_pred, torch.Tensor.cpu(output).detach().numpy()))
        all_labels += labels
        _, y_pred = output.max(dim=1)
        all_y_pred += y_pred
    ...
    print('get_dataloader time: {} | load_data time: {} | model time: {} '.format(time_end_load-start, end2-end1, end3-end2))
...
if __name__ == '__main__':
    ...
    if args.load_ckpt and args.attack:
        ...
        # graph_dict = pth.join('../load_data15_1473/graph_connections2.json')
        # graph_dict = util.read_dict_from_json(graph_dict)
        # label_correct, pred_score = calc_target_output(graph_dict)
        # # label_score, pred_score = calc_attack(old_graph_dict, new_graph_dict)
        # edge, label, value = alter_graph(graph_dict, bad_user_set)
