...
if True:   # parameter
    ...
    parser.add_argument('--freeze', default=True, type=str2bool, help='embedding freeze or not')
...
def calc_target_output(idx_graph, user_feats, graph_node_features, merged_tree_feature, merged_tree_edge_index, indx):
    model.eval()
    all_pred = []
    all_labels = []
    all_y_pred = []
...
def alter_graph(node_graph, index_graph, user_set, user_feats, graph_node_features, merged_tree_feature, merged_tree_edge_index, indx):
    """
    ...
    correct_label_origin, fake_value_origin = calc_target_output(index_graph, user_feats, graph_node_features, merged_tree_feature, merged_tree_edge_index, indx)
    ...
    for bad_user_node in tqdm(user_set[:3]):
        add_edge_flag = 0
        for tweet_node in tqdm(test_indices[:3]):   # all test tweet indices
            if int(bad_user_node) not in node_graph[str(tweet_node)] and int(tweet_node) not in node_graph[str(bad_user_node)]:
                ...
                correct_label_new, fake_value_new = calc_target_output(index_graph_new, test_labels)
        ...
        if add_edge_flag:        
            ...
            print('new correct_label num: {}'.format(correct_label_new))
            print('new fake value : {}'.format(fake_value_new))
    ...
    greedy_search_attack_trace[chosen_edge] = (correct_label_final, fake_value_final)
...
if __name__ == '__main__':
    ...
    train_loader, test_loader = get_dataloader(len(test_indices), train_indices, test_indices, adjdict)
    ...
    for data in test_loader:
        lgraph_node_features, loriginal_index_graph, luser_feats = Variable(data[0]).to(device), Variable(data[1]).to(device), Variable(data[3]).to(device)
        lmerged_tree_edge_index, lmerged_tree_feature, llabels = Variable(data[4]).to(device), Variable(data[5]).to(device), Variable(data[6]).to(device)
        lindx = data[7].to(device)
    ...
    if args.load_ckpt and args.attack:
        ...
        graph_node_features = copy.deepcopy(lgraph_node_features)
        original_index_graph = copy.deepcopy(loriginal_index_graph)
        user_feats = copy.deepcopy(luser_feats)
        merged_tree_edge_index = copy.deepcopy(lmerged_tree_edge_index)
        merged_tree_feature = copy.deepcopy(lmerged_tree_feature)
        labels = copy.deepcopy(llabels)
        indx = copy.deepcopy(lindx)

        # index_graph = original_index_graph.detach().clone()
        # node_graph = copy.deepcopy(adjdict)

        if True:
            # assert index_graph == original_index_graph
            model, optimizer, global_step, curr_epoch = _load_checkpoint()
            model.eval()
            graph_node_features1 = copy.deepcopy(lgraph_node_features)
            original_index_graph1 = copy.deepcopy(loriginal_index_graph)
            user_feats1 = copy.deepcopy(luser_feats)
            merged_tree_edge_index1 = copy.deepcopy(lmerged_tree_edge_index)
            merged_tree_feature1 = copy.deepcopy(lmerged_tree_feature)
            labels1 = copy.deepcopy(llabels)
            indx1 = copy.deepcopy(lindx)
            correct_label_origin, fake_value_origin = calc_target_output(original_index_graph1, user_feats1, graph_node_features1, merged_tree_feature1, merged_tree_edge_index1, indx1)
            print('origin correct_label: {}'.format(correct_label_origin))
            print('origin label_score: {}'.format(fake_value_origin))

            model, optimizer, global_step, curr_epoch = _load_checkpoint()
            model.eval()
            graph_node_features2 = copy.deepcopy(lgraph_node_features)
            original_index_graph2 = copy.deepcopy(loriginal_index_graph)
            user_feats2 = copy.deepcopy(luser_feats)
            merged_tree_edge_index2 = copy.deepcopy(lmerged_tree_edge_index)
            merged_tree_feature2 = copy.deepcopy(lmerged_tree_feature)
            labels2 = copy.deepcopy(llabels)
            indx2 = copy.deepcopy(lindx)
            correct_label_origin, fake_value_origin = calc_target_output(original_index_graph2, user_feats2, graph_node_features2, merged_tree_feature2, merged_tree_edge_index2, indx2)
            print('origin correct_label: {}'.format(correct_label_origin))
            print('origin label_score: {}'.format(fake_value_origin))

            model, optimizer, global_step, curr_epoch = _load_checkpoint()
            model.eval()
            graph_node_features3 = copy.deepcopy(lgraph_node_features)
            original_index_graph3 = copy.deepcopy(loriginal_index_graph)
            user_feats3 = copy.deepcopy(luser_feats)
            merged_tree_edge_index3 = copy.deepcopy(lmerged_tree_edge_index)
            merged_tree_feature3 = copy.deepcopy(lmerged_tree_feature)
            labels3 = copy.deepcopy(llabels)
            indx3 = copy.deepcopy(lindx)
            correct_label_origin, fake_value_origin = calc_target_output(original_index_graph3, user_feats3, graph_node_features3, merged_tree_feature3, merged_tree_edge_index3, indx3)
            print('origin correct_label: {}'.format(correct_label_origin))
            print('origin label_score: {}'.format(fake_value_origin))
        ...
        for k in range(1):
            node_graph, index_graph = alter_graph(node_graph, index_graph, bad_user_set, labels)
