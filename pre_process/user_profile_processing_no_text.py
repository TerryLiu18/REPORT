...
parser.add_argument('--filter', type=int, help='filter number', default=1)
args = parser.parse_args()
filter_num = args.filter
filter_num = 1

filtered_user_list = filtered_dataframe['user_id'].tolist()
user_profile_dir = pth.join('../datasets/twitter15/raw_data/user_profile')   # todo: right path
filtered_user_profile_path = pth.join('../load_data15/filtered_user_profile{}.csv'.format(str(filter_num)))
delete_tweet_list = txt2iterable(pth.join('../datasets/twitter15/auxiliary_data/delete_tweet_list.txt'))

if not pth.isdir('../datasets/twitter15/processed_data/processed_user_profile'):
    os.mkdir(pth.join('../datasets/twitter15/processed_data/processed_user_profile'))
...
with open(pth.join(filtered_user_profile_path), 'w', newline='', encoding='utf-8') as p1:
    ...
    for file in tqdm(available_tweet_list):
        ...
        processed_save_path = pth.join('../datasets/twitter15/processed_data/processed_user_profile', file)
