...
# tools.iterable2txt(delete_user_list, pth.join('../auxiliary_data/delete_user_list.txt'))
...
for f in tqdm(os.listdir(tree_tree_dir)):
    source_id = f.split('.')[0]
    tweet2node_dict[source_id] = node
    node += 1

tools.save_dict_to_json(tweet2node_dict, pth.join('../../../load_data16/tweet2node_dict.json'))