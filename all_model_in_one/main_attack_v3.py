...
@torch.no_grad()
def calc_target_output(index_graph, test_labels):
    model.eval()
    all_pred = []
    all_labels = []
    all_y_pred = []
...
def alter_graph(node_graph, index_graph, user_set, test_labels):
    """
    ...
    correct_label_current, fake_value_current = calc_target_output(index_graph, test_labels)
    print('current_correct_label: {}'.format(correct_label_current))
    print('current_label_score: {}'.format(fake_value_current))
    ...
    for bad_user_node in tqdm(user_set):
        add_edge_flag = 0
        for tweet_node in tqdm(test_indices):   # all test tweet indices
            if int(bad_user_node) not in node_graph[str(tweet_node)] and int(tweet_node) not in node_graph[str(bad_user_node)]:
                ...
                label_attack = correct_label_current - correct_label_new
                value_attack = fake_value_current - fake_value_new
    ...
    if best_label_attack > 0:
        chosen_edge = label_chosen_edge   # choose label_chosen_edge 
    elif value_chosen_edge is not None:   # choose value_chosen_edge 
    ...
    best_node_graph = node_graph_add_edge(node_graph, chosen_edge[0], chosen_edge[1])
    best_index_graph = index_graph_add_edge(index_graph, chosen_edge[0], chosen_edge[1])
    correct_label_final, fake_value_final = calc_target_output(best_index_graph, test_labels)

    greedy_search_attack_trace[chosen_edge] = (correct_label_final, fake_value_final)
    print(greedy_search_attack_trace)
...
if __name__ == '__main__':
    ...
    if args.load_ckpt and args.attack:
        ...
        for k in range(10):
            node_graph_new, index_graph_new = alter_graph(node_graph, index_graph, bad_user_set, labels)
