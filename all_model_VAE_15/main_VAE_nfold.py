...
parser.add_argument('--model', default='vae', type=str, help='ensemble/graph2tree/tree2graph/tree/graph')
...
def _train_model(train_indices, test_indices):
    ...
    for epoch in range(curr_epoch, args.epoches):
        ...
        for data in train_loader:
            ...
            # print(output)
            # print("label: ",labels)
            loss = loss_fun(output, labels) + rec_loss + 0.5*kl_loss
            loss.backward()
            optimizer.step()
            ## compute accuracy
    ...
    fw_log.write("BEST Accuracy: {:.4f}\n".format(early_stopping.best_accs))
...
def _testing_model(model, test_loader):
    ...
    for data in test_loader:
        ...
        merged_tree_edge_index, merged_tree_feature, labels = Variable(data[4]).to(device), Variable(data[5]).to(device), Variable(data[6]).to(device)
        indx = data[7].to(device)
        outputs, kl_loss, rec_loss = model(user_feats, graph_node_features, graph_edge_index, merged_tree_feature, merged_tree_edge_index, indx)
